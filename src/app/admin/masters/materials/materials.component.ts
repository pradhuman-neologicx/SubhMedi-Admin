...
@Component({
  ...
  export class MaterialsComponent {
    ...
    async Status(material_id: string, is_active: any) {
      ...
      this.employeeService.changesmaterialtatus(material_id, is_active)
    ...
    Getunitsfun() {
      this.employeeService.GetunitsApi().subscribe((response: any) => {
        if (response.status === 200) {
          ...
          this.unitsids = response.data.filter((item: any) => item.is_active == 1);
        }
      });
    }
    ...
    creatematerial() {
      ...
      if (this.materialcrealteform.valid) {
        ...
        this.material.description = this.materialcrealteform.get('Description')?.value;
        this.material.unit_id = this.materialcrealteform.get('selectUnits')?.value;
    ...
    materialupdate() {
      ...
      if (this.materialupdateform.valid) {
        ...
        this.material.description = this.materialupdateform.get('Description')?.value;
