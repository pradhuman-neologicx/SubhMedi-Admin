...
@Component({
  selector: 'app-waterexpense',
  templateUrl: './waterexpense.component.html',
  styleUrl: './waterexpense.component.scss'
})


  export class WaterexpenseComponent {
  
    @Input() watertype!: string;
    ...
    constructor(
     ...
     ngOnChanges(changes: SimpleChanges) {
      console.log('watertype updated:', this.watertype);
      if (changes['watertype'] && changes['watertype'].currentValue !== undefined) {
        // Initialization logic or any code you want to run when watertype updates
        console.log('watertype updated:', changes['watertype'].currentValue);
        this.initializeComponent(changes['watertype'].currentValue);
    ...
    // Initialization logic based on watertype changes
    private initializeComponent(watertypeValue: any) {
      // Place any logic that should run when watertype changes
      console.log('Initialization logic executed with watertype:', watertypeValue);
      // Add more initialization steps as needed
      if (watertypeValue!='0'|| watertypeValue !=0){
    ...
    console.log('Party ID:', this.partyID);
    console.log('watertype:', this.watertype);
  ...
  removeTask(index: number) {
   ...
   get materials(): FormArray {
    return this.watertypeform.get('materials') as FormArray;
  }
  ...
  addTask() {
   ...
   clearFileWithoutevent(): void {
    ...
    Getpartynamesubcontractorfun() {
      this.employeeService.getwaterParties(this.projectiD).subscribe((response: any) => {
    ...
    toggleADetails() {
      ...
      this.watertypeform.get('additionalcharges')?.clearValidators();
      if(this.showpaymentDetails){
        this.watertypeform.get('additionalcharges')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.watertypeform.get('additionalcharges')?.updateValueAndValidity();
   
    
    }
    toggleDDetails() {
      this.showdiscountDetails = !this.showdiscountDetails;
      this.watertypeform.get('Discount')?.clearValidators();
      if(this.showpaymentDetails){
        this.watertypeform.get('Discount')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.watertypeform.get('Discount')?.updateValueAndValidity();
    ...
    togglePAYDetails() {
      ...
      this.watertypeform.get('Payment')?.clearValidators();
      this.watertypeform.get('cheque')?.clearValidators();
      if(this.showpaymentDetails){
        this.watertypeform.get('Payment')?.setValidators([
          Validators.required,
        ]);
        this.watertypeform.get('cheque')?.setValidators([
          Validators.required,
        ]);
      }
    
      this.watertypeform.get('Payment')?.updateValueAndValidity();
      this.watertypeform.get('cheque')?.updateValueAndValidity();
    }
    toggleNDetails() {
      this.shownotesDetails = !this.shownotesDetails;
      this.watertypeform.get('Notes')?.clearValidators();
      if(this.showpaymentDetails){
        this.watertypeform.get('Notes')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.watertypeform.get('Notes')?.updateValueAndValidity();
    }
    toggleRDetails() {
      this.showreferenceDetails = !this.showreferenceDetails;
      this.shownotesDetails = !this.shownotesDetails;
      this.watertypeform.get('Reference')?.clearValidators();
      if(this.showpaymentDetails){
        this.watertypeform.get('Reference')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.watertypeform.get('Reference')?.updateValueAndValidity();
  ...
  Createaddotherexpense() {
    if (this.watertypeform.valid) {
      ...
      this.watertypeform.markAllAsTouched();
      this.selectedIn.emit('fill all the details Correctly');
      console.log(this.findInvalidControls(this.watertypeform));
    }
  }
  ...
  getbalanceparty() {
    this.employeeService.getbalance(this.projectiD,this.partyid).subscribe((response: any) => {
      if (response.status === 200) {
        ...
        this.watertypeform.get("Balance")?.setValue(response.parties!=undefined?response.parties.length>0?response.parties[0].balance:0:0)
      }
    });
  }
  partyid:any
...
this.getbalanceparty()
  ...
  calculatebalanceTotal(): number {
    ...
    const totaamount = parseFloat(this.watertypeform.get("Amount")?.value ?? '') || 0;
    const paymentout = parseFloat(this.watertypeform.get("Payment")?.value ?? '') || 0;
  ...
  calculateTotal(): number {
    ...
    const additionalCharge = parseFloat(this.watertypeform.get("additionalcharges")?.value ?? '') || 0;
    const discount = parseFloat(this.watertypeform.get("Discount")?.value ?? '') || 0;
  ...
  calculatePerMaterialAmount(index: number,type:any): number {
    ...
    const subtotal: number = parseFloat(watertypeform.get('subtotal')?.value ?? '0') || 0;
    // const unitsRange: number = parseFloat(watertypeform.get('unitsRange')?.value ?? '0') || 0;
    // const discount: number = parseFloat(watertypeform.get('discount')?.value ?? '0') || 0;
