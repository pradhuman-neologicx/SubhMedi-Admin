...
@Component({
  selector: 'app-supervisorpayment',
  templateUrl: './supervisorpayment.component.html',
  styleUrl: './supervisorpayment.component.scss'
})


  export class SupervisorpaymentComponent {
  
    @Input() Supervisortype!: string;
    Supervisortypeform!: FormGroup;
    @Input() partyId!: any;
    ...
    constructor(
     ...
     ngOnChanges(changes: SimpleChanges) {
      console.log('Supervisortype updated:', this.Supervisortype);
      if (changes['Supervisortype'] && changes['Supervisortype'].currentValue !== undefined) {
        // Initialization logic or any code you want to run when Supervisortype updates
        console.log('Supervisortype updated:', changes['Supervisortype'].currentValue);
        this.initializeComponent(changes['Supervisortype'].currentValue);
    ...
    // Initialization logic based on Supervisortype changes
    private initializeComponent(SupervisortypeValue: any) {
      // Place any logic that should run when Supervisortype changes
      console.log('Initialization logic executed with Supervisortype:', SupervisortypeValue);
      // Add more initialization steps as needed
      if (SupervisortypeValue!='0'|| SupervisortypeValue !=0){
    ...
    console.log('Party ID:', this.partyID);
    console.log('Supervisortype:', this.Supervisortype);
      ...
      this.Supervisortypeform = this.formBuilder.group({
  ...
  removeTask(index: number) {
   ...
   get materials(): FormArray {
    return this.Supervisortypeform.get('materials') as FormArray;
  }
  ...
  addTask() {
   ...
   clearFileWithoutevent(): void {
    ...
    Getpartynamesubcontractorfun() {
      this.employeeService.getsupervisorParties(this.projectiD).subscribe((response: any) => {
    ...
    toggleADetails() {
      ...
      this.Supervisortypeform.get('additionalcharges')?.clearValidators();
      if(this.showpaymentDetails){
        this.Supervisortypeform.get('additionalcharges')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.Supervisortypeform.get('additionalcharges')?.updateValueAndValidity();
   
    
    }
    toggleDDetails() {
      this.showdiscountDetails = !this.showdiscountDetails;
      this.Supervisortypeform.get('Discount')?.clearValidators();
      if(this.showpaymentDetails){
        this.Supervisortypeform.get('Discount')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.Supervisortypeform.get('Discount')?.updateValueAndValidity();
    ...
    togglePAYDetails() {
      ...
      this.Supervisortypeform.get('Payment')?.clearValidators();
      this.Supervisortypeform.get('cheque')?.clearValidators();
      if(this.showpaymentDetails){
        this.Supervisortypeform.get('Payment')?.setValidators([
          Validators.required,
        ]);
        this.Supervisortypeform.get('cheque')?.setValidators([
          Validators.required,
        ]);
      }
    
      this.Supervisortypeform.get('Payment')?.updateValueAndValidity();
      this.Supervisortypeform.get('cheque')?.updateValueAndValidity();
    }
    toggleNDetails() {
      this.shownotesDetails = !this.shownotesDetails;
      this.Supervisortypeform.get('Notes')?.clearValidators();
      if(this.showpaymentDetails){
        this.Supervisortypeform.get('Notes')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.Supervisortypeform.get('Notes')?.updateValueAndValidity();
    }
    toggleRDetails() {
      this.showreferenceDetails = !this.showreferenceDetails;
      this.shownotesDetails = !this.shownotesDetails;
      this.Supervisortypeform.get('Reference')?.clearValidators();
      if(this.showpaymentDetails){
        this.Supervisortypeform.get('Reference')?.setValidators([
          Validators.required,
        ]);
       
      }
    
      this.Supervisortypeform.get('Reference')?.updateValueAndValidity();
  ...
  Createaddotherexpense() {
    if (this.Supervisortypeform.valid) {
      ...
      for (let index = 0; index < this.materials.length; index++) {
        const Supervisortypeform =   this.materials.at(index);
  
    
      
        newMateria.push({
         
          "name": Supervisortypeform.get('name')?.value,
          "amount": Supervisortypeform.get('subtotal')?.value,
      ...
      formData.append("date", this.Supervisortypeform.get("date")?.value.toString());
      formData.append("party_id", this.Supervisortypeform.get("selectpartyname")?.value.toString());
      ...
      formData.append("notes", this.Supervisortypeform.get("Notes")?.value.toString());
      formData.append("reference_no", this.Supervisortypeform.get("Reference")?.value.toString());
      ...
      this.Supervisortypeform.markAllAsTouched();
      this.selectedIn.emit('fill all the details Correctly');
      console.log(this.findInvalidControls(this.Supervisortypeform));
    }
  }
  ...
  getbalanceparty() {
    this.employeeService.getbalance(this.projectiD,this.partyid).subscribe((response: any) => {
      if (response.status === 200) {
        ...
        this.Supervisortypeform.get("Balance")?.setValue(response.parties!=undefined?response.parties.length>0?response.parties[0].balance:0:0)
      }
    });
  }
  partyid:any
...
this.getbalanceparty()
  ...
  calculatebalanceTotal(): number {
    ...
    const totaamount = parseFloat(this.Supervisortypeform.get("Amount")?.value ?? '') || 0;
    const paymentout = parseFloat(this.Supervisortypeform.get("Payment")?.value ?? '') || 0;
  ...
  calculateTotal(): number {
    ...
    const additionalCharge = parseFloat(this.Supervisortypeform.get("additionalcharges")?.value ?? '') || 0;
    const discount = parseFloat(this.Supervisortypeform.get("Discount")?.value ?? '') || 0;
  ...
  calculatePerMaterialAmount(index: number,type:any): number {
    ...
    const subtotal: number = parseFloat(Supervisortypeform.get('subtotal')?.value ?? '0') || 0;
    // const unitsRange: number = parseFloat(Supervisortypeform.get('unitsRange')?.value ?? '0') || 0;
    // const discount: number = parseFloat(Supervisortypeform.get('discount')?.value ?? '0') || 0;
